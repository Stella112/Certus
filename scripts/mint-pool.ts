...
console.log(`Pool before: ${availableCount()} available (${POOL_FILE})`);
...
for (let i = 0; i < want; i++) {
  ...
  if (res.ok) {
    appendToPool({ address, customerId, cvRecordId: res.cvRecordId, mintedAt: new Date().toISOString() });
    minted++;
...
console.log(`\nMinted ${minted}/${want}. Pool now: ${availableCount()} available.`);
if (minted === 0) {
  console.log(
